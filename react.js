...
function App() {
  ...
  const getUserData = async () => {//create the async call to the api for userData
    const response = await fetch(url);
    const jsonData = await response.json();
    setUserData(jsonData)//set the userData variable using the setter
  }
  
  React.useEffect(() => {
    getUserData()//calls the api
  }, [])
